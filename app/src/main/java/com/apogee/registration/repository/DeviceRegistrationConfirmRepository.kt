                    ...
                    if (requestBody != null) {
                        ...
                        try {
                            ...
                            if (checkVaildString(deviceRegResponse)) {
                                ...
                                _data.value =
                                    if (deviceRegResponse.contains("Registration Completed", true)
                                    ) {
                                        val split = deviceRegResponse.split(",".toRegex()).let {ls->
                                            ls.subList(1, ls.size - 1)
                                        }
                                        DataResponse.Success(split)
                                    } else {
